...
=======
src/stepopt/constrained/optimisers.py
"""
...
from stepopt.core.oracle import FirstOrderOracle, Oracle
from stepopt.core.stopping import StoppingCriterion, StoppingCriterionType
from stepopt.functions import ConvexQuadratic
from stepopt.functions.protocol import ZeroOrderFunctionProto
from stepopt.problems import (
    EqualityConstrainedQuadraticProgram,
    InequalityConstrainedQuadraticProgram,
)
from stepopt.types import Matrix, Scalar, Vector, dtype
...
class EQPSolver:
    ...
    def solve(
        ...
        try:
            sol = Vector(np.linalg.solve(KKT, rhs))
...
class ActiveSetMethod(
    ...
    def direction(
        ...
        if len(W) == 0:  # No active constraints
            ...
            b_eq = Vector(np.zeros((0,), dtype=dtype))
        else:
            A_eq = Matrix(A[W, :])
            b_eq = Vector(np.zeros((len(W),), dtype=dtype))
