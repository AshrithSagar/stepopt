...
class Logger:
    ...
    def configure(
        ...
        Parameters:
            level: `int` or `str` name (e.g. "DEBUG", "INFO") or `None` (uses `stepopt_LOG_LEVEL` env or `WARNING`).
            name: `str` name of the logger.
            force: `bool`, if `True`, re-configure even if already configured.
        """
        logger = logging.getLogger(name)
        lvl = level if level is not None else os.getenv("stepopt_LOG_LEVEL", "WARNING")
    ...
    @classmethod
    def get(
        cls, name: str = "stepopt", ensure_configured: bool = True
    ) -> logging.Logger:
        """
